...
\begin{proof}
	...
	By this contradiction we have that along the null generators \(U_{\mu}\mathrm{H}^{\mu} > 0\), and the conclusion is reached exactly as in \ref{th:classical-bh-area}.
\end{proof}
...
\begin{proof}
	...
	For the null generators leaving on \(\partial J^+(K)\) equation \eqref{eq:contradiction-generalized-area} implies that there exist \(\ell > 0\) and \(f\in C^{\infty}_{1,0}[0, \ell]\) so that the condition \(\eqref{eq:fp-criteria}\) is satisfied. But this leads to a contradiction in the same way as in theorem \ref{th:classical-bh-area} because we are again in a globally hyperbolic spacetime, and null generators are not allowed to contain any focal point.
...
\end{lemma}
	...
	\]
	\begin{remark}
		From the above procedure it is clear that whenever we choose a class of funtcions \(\mathcal{A}\) we can always refer to its closure \(\bar{\mathcal{A}}\) and the all procedure remains unaffected. Consequentially, limit functions can be always considered included in the sets \(\mathcal{A}\) that we pick.
	\end{remark}
...
\begin{SCfigure}
	...
	\caption[]{plot of \(\mathcal{V}_{\beta}(\ell_0, \ell)\) (\(\hat{z}\) axis). On the left it appears part of the \(\ell\) axis, while the hidden one corresponds to \(\ell_0\). The colorscale should help to spot out the minimum of the visualized sheet.}
	\label{fig:V-beta}
\end{SCfigure}
...
The first optimization has been already performed in \cite{fewster2020new}, even if in their computation they neglect some terms that instead will be relevant for us, because of the different regime of approximation that we will need to set. Let us do that first:

\begin{prop}
    The bound optimized on \(\ell \ge \ell_0\) is
    \[
      (n-2)\mathcal{V}_{\beta}(\ell_0) = (A_m - 1)\rho_0\ell_0 + \frac{Q_mC_m}{\ell_0} + 2 \sqrt{(n - 2)A_mB_mQ_0}   
    \]
    and is obtained for:
    \[
    \tilde{\ell} = \ell_0\left(1 + \sqrt{\frac{(n - 2) B_m}{A_m Q_0\ell_0^2}}\right)    
    \]
\end{prop}
...
\begin{proof}
    ...
    The interesting case presents when \(\mu \ge 1\), for which \(\frac{1}{\mu^{2m - 1}} \le \frac{1}{\mu} \) (in particular in subsection \ref{subsec:non-min-EKG-theory} we will focus on the \(m = 1\) case, where the equality holds). The expression in \(\eqref{eq:J-sobolev-condition}\) reduces to:
    \begin{align*}
...
\begin{prop}
	...
	\begin{equation}
		...
		\mathcal{V}_{\beta} = \frac{5}{6}\sqrt{\frac{3}{2}Q_m \vert\rho_0\vert} + 2 \sqrt{(n - 2)A_mB_mQ_0}
	\end{equation}

	corresponding to \(\tilde{\ell}_0^2 = - \frac{3Q_m}{2\rho_0}\)
\end{prop}
\begin{proof}
...
It may attract our attention the curious feature that the minimum of \(\mathcal{V}_{\beta}\) lays at finite \(\ell\): a simple heuristic argument instead suggests that a prioiri we should expect it to be at \(\ell \rightarrow +\infty\).
...
\begin{proof}
	...
	\end{align*}
	The second inequality can be worked out in a similar way, being careful that this time there is an angular dependance because of the factor \((k_{\mu}U^{\mu})^2\) originated by the derivatives. This factor adds a \(k^2\) power to the radial part of the integral, and a factor:
	\[
...
By substituting \(\rho_0\) into \(\eqref{eq:minimized-V-beta}\), and recovering the temperature dependence, we finally get:
\begin{equation}
