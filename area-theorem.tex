...
\]
we then want to compute \(Q\) and \(\tilde{Q}\) in terms of some physical quantities: for this reason we pick a quantized scalar field in a thermal state of temperature \(T\); we find the value for the Wick square of the massless scalar field in a Kubo-Martin-Schwinger (KMS) state with temperature \(T\) in section \(5.2\) of \cite{fewster2020new}, and we can compute it in a way analogous to what has been done in the final appendix of \cite{brown2018singularity}.
...
\begin{prop}
    The Wick square of the massless field in a KMS thermal state of temperature \(T\) within a \(n\)-dimensional spacetime is given by:
    \begin{align}
        \phi_{max}^2 \sim \langle \colon \phi^2 \colon\rangle_T &= \frac{T^{n - 2}}{2^{n - 2}\pi^{\frac{n - 1}{2}}}\frac{\Gamma(n - 2)}{\Gamma\left(\frac{n - 2}{2}\right)}\zeta(n) \\
        \sup_{\gamma}\vert \nabla_U\phi\vert ^2 \sim \langle \colon (\nabla_U\phi)^2 \colon\rangle_T &= \frac{T^{n}}{3\cdot2^{n - 4}\pi^{\frac{n - 1}{2}}}\frac{\Gamma(n)}{\Gamma\left(\frac{n - 1}{2}\right)}\zeta(n)
    \end{align}

\end{prop}
    
\begin{proof}
    \VS{AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA there is a sort of Bose-Einstein condensation.}
        %todo: trova e scrivi la dimostrazione
\end{proof}
Restoring the units and reducing to \(n = 4\) spacetime dimension we get:
\begin{align}
    Q_1 &= \underbrace{\frac{4\pi\sqrt{\pi}\xi}{3}}_{\alpha} \left(\frac{T}{T_{Pl}}\right)^2 \\
...
So far we have derived a class of bounds \(\mathcal{V}(\ell, \ell_0)\) that depend on \(\ell\) and \(\ell_0\). Among them, as all of them need to be obeyed, the most interesting is the most restrictive, because it's the one that gives as the most information.
...
\begin{prop}
    ...
    and is obtained for:
    \[
    \ell = \ell_0(1 + \sqrt{\frac{(n - 2) B_m}{A_m Q_0\ell_0^2}})    
    \]
\end{prop}

\begin{proof}
    The proof is very similar to the one given in \cite{fewster2020new}, but we have to pay attention to keep all the terms.

    Set \(\ell = \ell_0(1 + B_m\mu)\), so that the constraint \(\ell\ge\ell_0\) becomes simply \(\mu \ge 0\).
