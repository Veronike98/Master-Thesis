...
\begin{proof}
	...
	By contradiction, suppose instead that \(\mathrm{H}^{\mu}U_{\mu} < 0\) at \(p\). Let \(\Sigma_1\) be any Cauchy hypersurface for \(\tilde{V}\) through \(p\), and call \(\mathscr{H}_1 = H \cap \Sigma_1\).
	As \(\mathrm{H}^{\mu}U_{\mu}(p) < 0\) in a neighborhood of \(p\) we can deform \(\mathscr{H}_1\) outword on \(\Sigma_1\), to \(\mathscr{H}_1'\), such that
	\[
	\begin{cases}
	...
	\end{cases}
	\]
	where, called \(K\) the closed region in \(\Sigma_1\) between \(\mathscr{H}_1\) and \(\mathscr{H}_1'\), \(U'^{\mu}\) is the tangent vector of the null geodesic generator of \(\partial J^+(K)\).
	
	Pick a point \(q \in \mathscr{I}^+ \cap \partial J^+(K)\):  (\VS{This exists for sth similar to 12.2.6 of Wald})
	...
	\end{cases}
	\]
	and %todo: sistemare il segno
	\[
	\int_{0}^{\ell} \big((n -2)(\nabla_Uf)^2 - f^2R_{\mu\nu}U^{\mu}U^{\nu} \big)d\lambda\le 
	- \frac{n -2}{\ell} \le -(n -2) f^2 U_{\mu} \mathrm{H}^{\mu} \le
