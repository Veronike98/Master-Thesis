...
We have used \(g^{\mu\alpha} = U^{\mu}W^{\alpha} + U^{\alpha}W^{\mu} - \sum_{i=1}^{n - 2}E_i^{\mu}E_i^{\alpha}\) (with \(W^{\mu}\) a suitable null vector, such that \(W^{\mu}U_{\mu} = 1\) and \(W_{\mu}E_i^{\mu} = 0\), see \ref{lemma:metric-decomposition} for a proof), which means:
	...
	\]
	\begin{remark}
		Here we have decomposed \(B_{\mu\nu}\) in its spin \(0, 1\) and \(2\) components, but we are not talking about the represantation of the usual Lorentz group: here we need \(\theta\) to be a singlet for any linear transformation that leaves \(- \sum_{i=1}^{n - 2}E_i^{\mu}E_i^{\nu}\) invariant. In particular, any contraction or dilatation in the \(U^{\mu}\) or \(W^{\mu}\) direction is part of the group we are representing.
		
		This, in some sense, is a way of saying that we care about a larger group of transformations: the important features is that they are isometries on \(P\), but the normal geodesics can be reparametrized, contracted or dilatated.
	\end{remark}
	...
	\begin{proof}
		...
		\end{align*}
		Now, simply plugging in \eqref{eq:B-decomposition}, and using the simmetries and \eqref{eq:trace-shear} and \eqref{eq:trace-vorticity}, one easily obtains \eqref{eq:Raychadhuri-theta}.
		...
		\]
		Finally the mixing term of shear and vorticity - using the antisimmetry of \(\omega_{\mu\nu}\) - simply corresponds to 
		\[
	...
	\begin{proof}
		...
		g^{\mu\nu} = U^{\mu}W^{\nu} + U^{\nu}W^{\mu} - \sum_{i=1}^{n - 2}E_i^{\mu}E_i^{\nu}
		\]
		This means that the transversial field of the congruence, that must be a \(P\)-Jacobi field, can be written as:
		\[
		V^{\mu} = V^iE_i^{\mu} \quad \text{ where } \quad V^i = E^i_{\mu}V^{\mu};
		\]
		moreover we already know that \(\frac{D}{D\lambda}V^{\mu} = B\indices{^{\mu}_{\nu}}V^{\nu}\), so expanding in the above mentioned basis, and remembering that the \(\{E_i^{\mu}\}\) are parallell transported, we get:
		\begin{equation}
		...
		\]
		It is a well-known fact of linear algebra that this implies the following differential equation for the determinant: %(provided \(A\) is non singular) forse non serve. Se serve il Wald sistema a pag. 226.
		\begin{equation}
		\label{eq;det-A}
		...
		\begin{itemize}
			\item[\ding{99}] If \(\theta = \texttt{tr}\mathrm{B}\) is bounded in \([0, \lambda_0]\), there mustn't be any \(\lambda \in [0, \lambda_0]\) for which \(\texttt{det}A(\lambda) = 0\); otheriwse \(\texttt{det}A = 0\) on the all interval, contradicting the fact that \(A(0) = \mathbb{1}\). It follows that \(\texttt{det}A\) can vanish for \(\lambda \rightarrow \lambda^{\star}\) only if \(\vert\texttt{tr}\mathrm{B}\vert \rightarrow \infty\) for \(\lambda \rightarrow \lambda^{\star}\).
			...
			\item[\ding{99}] At the same time, substituiting the definition of \(A\) into \eqref{eq:Jacobi-E_i} one gets
			\[
		...
		\]
		But saying \(\texttt{det}A(\lambda^{\star}) = 0 \) is equivalent to stating that \(A(\lambda^{\star}) \) has non trivial kernel, and hence there exists a non trivial initial data \(V^{\mu}(0)\) for which \(V^{\mu}(\lambda^{\star}) = 0\). This is exactly the definition of \(\gamma(\lambda^{\star})\) being a focal point, so the proof id concluded.
	\end{proof}

	Proposition \ref{prop:fp-criteria-Raychaudhuri} is establishing another criterion for the existence of focal points, which in the proofs of any relativity theorem will play the same role as proposition \ref{prop:fp-criteria}. In this context, studying \ref{eq:Riccati-ineq} has the same effect as analyzing \ref{eq:fp-criteria}, because they are the equations that constrain the quantities of intrests for the existence criteria of focal points, and thus will provide us with quantitive information about what conditions we need for their existence.
	...
	Having said that, what stated above is only a broad intuition about why we should prefer one method to the other, but it's worthwhile understanding more deeply the interplay bewteen them, in order to better realize the strenghts and the weaknesses of both.
